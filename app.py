...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "postgresql://postgres:synergie1234@localhost/synergie"
)
...
class Data_count(db.Model):
    ...
    events = db.relationship("Event", back_populates="data_count")
...
class Event(db.Model):
    ...
    data_count_id = db.Column(db.Integer, db.ForeignKey("data_count.id"))
    data_count = db.relationship("Data_count", back_populates="events")
...
def query_max_values(start_time, end_time):
    total_billet_count = (
        db.session.execute(
            text(
                """SELECT Max(billet_count) AS total_billet_count
                        FROM data_count
    ...
    total_cobble_count = (
        db.session.execute(
            text(
                """SELECT Max(cobble_count) AS total_cobble_count
                        FROM data_count
                        WHERE time_stamp >= :start_time AND time_stamp < :end_time;"""
            ),
            {"start_time": start_time, "end_time": end_time},
    ...
    total_energy_consume = (
        db.session.execute(
            text(
                """SELECT Max(energy_consumption) AS total_energy_consume
                        FROM data_count
                        WHERE time_stamp >= :start_time AND time_stamp < :end_time;"""
            ),
            {"start_time": start_time, "end_time": end_time},
        ...
        with app.app_context():  # Ensure this code runs within the application context
            if machine_running and int_data[1] == 3 and not cobble_produced:
                if int_data[2] == 4 and not cobble_produced:
                    ...
                    plc_data_processed["event_name"] = "cobble in chopping area ch1"
                    plc_data_processed["event_start"] = datetime.now()
                    ...
                    cobble_produced = True
                    plc_data_processed["cobble_count"] += 1
                    plc_data_processed["event_name"] = "cobble in chopping area ch2"
                    plc_data_processed["event_start"] = datetime.now()
                    # plc_data_processed["energy_consumption"] = int_data[6]
                    # plc_data_processed["fuel_consumption"] = int_data[7]
                    plc_data_processed["event_area"] = "CH2"
                    ...
                    cobble_produced = True
                    plc_data_processed["cobble_count"] += 1
                    plc_data_processed["event_name"] = "cobble in chopping area cb"
                    plc_data_processed["event_start"] = datetime.now()
                    # plc_data_processed["energy_consumption"] = int_data[6]
                    # plc_data_processed["fuel_consumption"] = int_data[7]
                    plc_data_processed["event_area"] = "CB"
        ...
        with app.app_context():
            ...
            if int_data[5] == 7:
                ...
                last_cobble_count_result = db.session.execute(
                    text(
                        "SELECT cobble_count FROM public.data_count ORDER BY time_stamp DESC LIMIT 1"
                    )
                ).scalar()  # Fetch the single value
...
def fetch_hourly_data(date):
    ...
    with app.app_context():
        ...
        while start_time < end_time:
            ...
            # Extract relevant counts for cobble, billet, and energy
            ...
            first_cobble_count = filtered_data[0].cobble_count if filtered_data else 0
            last_cobble_count = filtered_data[-1].cobble_count if filtered_data else 0

            first_energy_consumption = (
                filtered_data[0].energy_consumption if filtered_data else 0
            )
            last_energy_consumption = (
                filtered_data[-1].energy_consumption if filtered_data else 0
            )
            ...
            hourly_data["cobble"][hour_range] = (first_cobble_count, last_cobble_count)
            hourly_data["energy"][hour_range] = (
                first_energy_consumption,
                last_energy_consumption,
            )
        ...
        # Calculate differences for billet, cobble, and energy counts
